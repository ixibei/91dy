...
<div id="contentwrapper" class="contentwrapper">
    ...
    <div class="tableoptions">
        ...
        <button class="radius3" onclick="option('news_class','{{URL::to('base/option')}}')">应 用</button>
    </div>
  ...
  <table cellpadding="0" cellspacing="0" border="0" class="stdtable stdtablecb">
    ...
    <tr class="group">
      ...
      <td>{{$val['filename']}}</td>
      <td class="center">{{ HTML::link(URL::action("Admin_NewsController@getEditCategory",array($val['id'])), '编辑') }}</td>
    </tr>
        ...
        @foreach($val['children'] as $k=>$v)
        	    <tr>
                  ...
                  <td>{{$v['filename']}}</td>
                  <td class="center">{{ HTML::link(URL::action("Admin_NewsController@getEditCategory",array($v['id'])), '编辑') }}</td>
                </tr>
	    @endforeach        
        @endif
    @endforeach
