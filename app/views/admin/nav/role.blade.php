...
<div id="contentwrapper" class="contentwrapper">
    ...
    <div class="tableoptions">
        ...
        <button class="radius3" onclick="option('admin_roles','{{URL::to('base/option')}}')">应 用</button>
    </div>
  ...
  <table cellpadding="0" cellspacing="0" border="0" class="stdtable stdtablecb">
    ...
    <tr @if($key % 2 == 1) class="group" @endif>
      ...
      <td>{{$val->id}}</td>
      <td>{{$val->title}}</td>
      <td onclick="changeStatus(this,{{$val->id}},'admin_roles','{{URL::to('base/changeStatus')}}')"><img src="/img/admin/status_{{$val->status}}.png" class="status"/></td>
      <td class="center">{{ HTML::link(URL::action("Admin_NavController@getEditRole",array($val->id)), '编辑') }}</td>
    </tr>
    @endforeach
      </tbody>
  </table>
