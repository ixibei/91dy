...
<div id="contentwrapper" class="contentwrapper">
    ...
    <div class="tableoptions">
        ...
        <button class="radius3" onclick="option('ht','{{URL::to('base/option')}}')">应 用</button>
  ...
  <table cellpadding="0" cellspacing="0" border="0" class="stdtable stdtablecb">
    ...
    <tr @if($key % 2 == 1) class="group" @endif>
      ...
      <td class="center"><a href="{{$baseUrl}}huati/{{$val->filename}}.html" target="_blank">{{$val->ftname}}</a></td>
      <td class="center">{{$val->hits}}</td>
      <td class="center">{{$val->adduser}}</td>
      <td onclick="changeStatus(this,{{$val->id}},'ht','{{URL::to('base/changeStatus')}}','tj')"><img src="/img/admin/status_{{$val->tj}}.png" class="status"/></td>
      <td class="center">{{ HTML::link(URL::action("Admin_TopicController@getEdit",array($val->id)), '编辑') }}</td>
    </tr>
    @endforeach
      </tbody>
  </table>
...
function searchKeywords(obj){
	...
	var url = "{{ url('topic/index') }}?keywords="+keywords;
	location.href = url;
}
</script>
@stop