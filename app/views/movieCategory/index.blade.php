        ...
        <div class="af-hd cf">
            ...
            <div class="selected">
                ...
                <?php 
					...
					$bread[$key] = 0;
				?>
                <span class="c">{{ $val }}<a href="{{ URL::route('movieCategorySearch', [$uri,$bread[0],$bread[1],$bread[2],$bread[3],$bread[4]] ) }}" class="close">×</a></span>
                @endforeach
        ...
        <div class="af-list">
            <dl class="cf">
                ...
                <dd>
                	...
                	<span class="c"><a href="{{ URL::route('movieNewsSearch', [$param[0],$param[1],$param[2],1] ) }}">最新</a></span>
            ...
            <dl class="cf">
                <dt>按地区</dt>
                <dd>
 	                <span class="c @if($param[0] == 0) on @endif"><a href="{{ URL::route('movieCategorySearch', [$uri,0,$param[1],$param[2],$param[3],1] ) }}">全部</a></span>
                   	@foreach($country as $key=>$val)
    	            <span class="c @if($val->id == $param[0]) on @endif"><a href="{{ URL::route('movieCategorySearch', [$uri,$val->id,$param[1],$param[2],$param[3],$param[4]] ) }}">{{ $val->name }}</a></span>
                    @endforeach
                </dd>
            </dl>
            <dl class="cf">
                <dt>按年份</dt>
                <dd>
                    <span class="c @if($param[1] == 0) on @endif"><a href="{{ URL::route('movieCategorySearch', [$uri,$param[0],0,$param[2],$param[3],1] ) }}">全部</a></span>
                    <?php $arr = [2015,2014,2013,2012,2011,2010,2009,2008,2007,2006,2005,2004,2003,2002,2001,2000];?>
					@foreach($arr as $val)
                    <span class="c @if($val == $param[1]) on @endif"><a href="{{ URL::route('movieCategorySearch', [$uri,$param[0],$val,$param[2],$param[3],$param[4]] ) }}">{{ $val }}</a></span>
                    @endforeach
                </dd>
            </dl>
            <dl class="star-list cf">
                <dt>按明星</dt>
                <dd>
                <span class="c @if($param[2] == 0) on @endif"><a href="{{ URL::route('movieCategorySearch', [$uri,$param[0],$param[1],0,$param[3],1] ) }}">全部</a></span>
                    @foreach($mingxing as $key=>$val)
                    <span class="c @if($val->id == $param[2]) on @endif"><a href="{{ URL::route('movieCategorySearch', [$uri,$param[0],$param[1],$val->id,$param[3],$param[4]] ) }}">{{ $val->name }}</a></span>
                    @endforeach                
                </dd>
            </dl>
        </div>
                	...
                	<div class="cell cf">
                       ...
                       <div class="infor">
                            ...
                            <dl class="cf">
                                <dt>主演：</dt>
                                <dd>
                                    @foreach(MoviePerson::select('P.name','P.id')->where('P.id','!=','')->where('movie_id','=',$val->id)->leftJoin('m_person as P','P.id','=','m_movie_person.person_id')->take(3)->get() as $v)
