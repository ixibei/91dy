...
jQuery(document).ready(function(){
	...
	// 子菜单点击展开
	jQuery('.vernav > ul li a, .vernav2 > ul li a').live('click',function(){
	...
	// 子菜单点击效果
	jQuery('.vernav > ul li ul li, .vernav2 > ul li ul li').live('click',function(){
		jQuery.cookie('currentUrl',null);
		jQuery('.vernav > ul li, .vernav2 > ul li').removeClass('current');
		...
		jQuery.cookie('currentUrl',href,{ path: '/' });
	});
	...
	//收缩菜单栏
	jQuery('.togglemenu').live('click',function(){
	...
	//头部导航切换效果
	jQuery(".headermenu li").live('click',function(){
		var index = jQuery(this).index();
		jQuery(".vernav2").hide();
		jQuery(".vernav2").eq(index).show();
		jQuery(this).addClass('current');
		jQuery(this).siblings().removeClass('current');
	})

	//表格滑动效果
	jQuery('.stdtable tr').click(function(){
		$(this).siblings().removeAttr('id');
		$(this).attr('id','blue-xt');
